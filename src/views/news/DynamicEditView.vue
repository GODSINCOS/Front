...
import { getNewsDetail, editNewsItem, uploadNewsImage, getNewsImageUrl } from '@/api/news'
  ...
  try {
    ...
    if (res.success) {
      ...
      if (res.data && res.data.id) {
        imagePreviewUrl.value = getNewsImageUrl(res.data.id)
      }
    }
  } catch (error) {
...
const handleSubmit = () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        const res = await editNewsItem(id, {
          ...form.value,
          imageUrl: undefined // 后端不再用imageUrl字段
        })
        if (res.success) {
          // 上传图片
          if (imageFile.value) {
            await uploadNewsImage(id, imageFile.value)
          }
